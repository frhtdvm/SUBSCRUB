...
const PLAID_BROKER_BASE_URL =
	process.env.EXPO_PUBLIC_PLAID_BROKER_BASE_URL ?? "";
...
const CREDENTIAL_REQUIREMENTS = [
	{
		key: "EXPO_PUBLIC_PLAID_BROKER_BASE_URL",
		label: "Plaid Broker URL",
		service: "Bank transactions",
		configured: PLAID_BROKER_BASE_URL.length > 0,
	},
	{
		key: "EXPO_PUBLIC_GOOGLE_CLIENT_ID_IOS",
		label: "Google iOS Client ID",
		service: "Gmail receipts on iPhone/iPad",
		configured: GOOGLE_CLIENT_ID_IOS.length > 0,
	},
	{
		key: "EXPO_PUBLIC_GOOGLE_CLIENT_ID_ANDROID",
		label: "Google Android Client ID",
		service: "Gmail receipts on Android",
		configured: GOOGLE_CLIENT_ID_ANDROID.length > 0,
	},
	{
		key: "EXPO_PUBLIC_MICROSOFT_CLIENT_ID",
		label: "Microsoft Client ID",
		service: "Outlook receipts",
		configured: MICROSOFT_CLIENT_ID.length > 0,
	},
	{
		key: "EXPO_PUBLIC_REVENUECAT_API_KEY",
		label: "RevenueCat API Key",
		service: "In-app purchases",
		configured: REVENUECAT_API_KEY.length > 0,
	},
];
...
export default function SettingsScreen() {
	...
	const currentJurisdiction: Jurisdiction = (store.settings?.jurisdiction ??
		"GENERIC") as Jurisdiction;
			...
			<ScrollView contentContainerStyle={{ padding: 20, paddingTop: 28 }}>
					...
					{store.isDemoMode ? (
						<View style={{ padding: 14 }}>
							<Text
								...
								Production mode requires the following env vars in your EAS
								build:
							</Text>
						...
						<View style={{ padding: 14 }}>
							<Text
								...
								Connected to real sources. Scans use live bank data and email
								receipts.
							</Text>
						</View>
					)}
				...
				<SettingsSection title="ACCOUNT">
					...
					<SettingsRow
						label="Last Scan"
						value={
							store.profile?.lastScanDate
								? formatDate(store.profile.lastScanDate)
								: "Never"
						}
					/>
					<SettingsRow
						...
						value={
							store.profile
								? `$${store.profile.totalSaved.toFixed(2)}`
								: "$0.00"
						}
						last
					/>
					...
					{store.connections.length === 0 ? (
						...
						store.connections.map((conn) => (
							<SettingsRow
								key={conn.id}
								label={
									conn.source.charAt(0).toUpperCase() + conn.source.slice(1)
								}
						...
						<TouchableOpacity
							...
							<View>
								<Text
									style={{
										color:
											currentJurisdiction === j ? Colors.primary : Colors.text,
										fontSize: 13,
										fontFamily: "SpaceMono",
										fontWeight: currentJurisdiction === j ? "700" : "400",
									}}>
									{j}
								</Text>
								<Text
