...
export async function connectOutlook(): Promise<ConnectorResult<OAuthConnectResult>> {
  if (!isOutlookConfigured()) {
    return { success: false, error: 'Microsoft OAuth not configured', isDemo: true };
  }
  ...
  try {
    ...
    await SecureStore.setItemAsync(OUTLOOK_ACCESS_TOKEN_KEY, tokens.access_token);
    if (tokens.refresh_token) {
      await SecureStore.setItemAsync(OUTLOOK_REFRESH_TOKEN_KEY, tokens.refresh_token);
    }
...
export async function syncOutlookEmails(
  ...
  if (!accessToken) return { success: false, error: 'Not connected to Outlook' };
