...
const GOOGLE_CLIENT_ID_FOR_PLATFORM =
	Platform.OS === "ios"
		? GOOGLE_CLIENT_ID_IOS || GOOGLE_CLIENT_ID
		: GOOGLE_CLIENT_ID_ANDROID || GOOGLE_CLIENT_ID;
...
export function isGmailConfigured(): boolean {
	return GOOGLE_CLIENT_ID_FOR_PLATFORM.length > 0;
}
	...
	try {
		...
		const tokenResp = await fetch("https://oauth2.googleapis.com/token", {
			...
			body: new URLSearchParams({
				...
				client_id: GOOGLE_CLIENT_ID_FOR_PLATFORM,
